...
tool 20-sim.  Both Crescendo and Overture are 
...
The Crescendo tool was originally called the DESTECS tool since it was produced in the European Seventh Framework research project called DESTECS (see Section~\ref{sec:destecs} below). It supports a method of collaborative modelling and simulation called \emph{co-simulation}. 
...
that combine Continuous-Time (CT) models with Discrete-Event (DE)
...
The Crescendo tool is one of a family of tools with common shared code.
...
This user manual explains how to use the Crescendo IDE for developing collaborating models
 (co-models) and analysing them. In essence it is structured in 2 parts: The first part provides the basics for getting started using the Crescendo tool and the second part acts as a reference manual for the Crescendo tool. The first part contains Chapter~\ref{chap:introconcepts} introducing the basic Crescendo concepts\footnote{Appendix~\ref{app:Glossary} provides a complete list of the Crescendo common concepts.};
Chapter~\ref{chap:getting} explans how to get hold of the software; and finally
Chapter~\ref{chap:gettingstarted} explains how to quickly get started using the Crescendo tool with existing models that can be imported directly. Afterwards the second part also contains four chapters explaining the main possibilities in the Crescendo tool. Chapter~\ref{chap:basic} explains the different views in the Crescendo tool, its editors and its way of handling projects. Chapter~\ref{chap:cosim} explains the co-simulation possibilities. This is followed by Chapter~\ref{chap:DSE} which provides information about how co-simulation can be extended with exploring the candidate design space. Finally this part is completed in Chapter~\ref{chap:postana} explaining the post-analysis possibilities in particular relevant to exploratory situations.
...
users who are new to Crescendo.
...
called a \emph{Discrete-Event simulation}. There are no accuracy issues
involved and therefore no backstepping is required.

The properties of a model~that affects its behaviour, but which remain
...
additional licence. 20-sim is a commercial tool that will run in
Viewer mode on any computer. This means that you
can only run and edit models! If you want to save changes to your model, you will need
a licence. You can send an email to
Controllab \url{mailto://info@controllab.nl} to get a trial licence.
...
the licence dialog will be opened.

\begin{figure}[htbp]
\centering
\includegraphics[width=.4\textwidth]{images/20simLicense.png}
\caption{Licence for 20-sim.}
\end{figure}
...
%% \caption{Message if you do not have a licence for 20-sim.}
%% \end{figure}
...
The Crescendo \emph{Editor View} allows you to edit VDM files, Contracts and Scenarios and it highlights the different keywords. Figure~\ref{fig:editorcontract1} shows how a Crescendo contract looks in the editor (and in a different pane a VDM-RT file).
...
The link file is automatically created when you start a new
project. You can edit the link file by selecting the configuration
folder in the project tree. Expand the project and configuration
folder and \emph{select the file} \texttt{vdm.link}.
...
\chapter{Co-Simulation}\label{chap:cosim}
...
that are logged during the co-simulation. At the time this manual
was written there is only the possibility of logging DE variables;
...
\chapter{Design Space Exploration}\label{chap:DSE}

In order to support Design Space Exploration (DSE), the Automated
Co-model Analysis (ACA) feature can automatically running many different
...
\centering
\includegraphics[width=.6\textwidth]{images/DestecsACAdebugconfig.png}
\caption{Creating an ACA debug configuration.}
...
Unless you want to edit the class files, leave \emph{Use linked libraries} option
...
type, instantiate it with a constructor, call ``\texttt{SetSampleTime}'' and then
call ``\texttt{Output}'' in your control loop. All of the control library classes
have an operation called \texttt{Output}, which takes in an error and
...
\chapter{Post-Analysis}\label{chap:postana}
...
The Octave project on Source Forge provides an easy installable Windows installer for Octave: \url{http://octave.sourceforge.net/}. Furthermore a Octave GUI is provided by \url{http://www.gnu.org/software/octave/} which provides a workspace and editor that can be used to assist the user of Crescendo in creating custom scripts.
...
The Octave script will contain the necessary code to interpret the logged variables and functions to generate a plot. However, it will not contain the command to display the plot. This setting will insert the plot commend in the script and automatically show a plot when executed by Ocatve. If the script is to be included in a custom script written by the user simply as data access this option should be disabled.

The option is available in the debug configuration that affects
...
the results are easy to compare and inspect visually. 
...
Figure~\ref{fig:octaveplotaca} shows the ACA launch (Main Tab)
...
navigating to the \emph{Window} $\rightarrow$ \emph{Preferences} menu as shown in Figure~\ref{fig:OctaveSetting}.

\begin{figure}[htbp]
\centering
\includegraphics[width=.6\textwidth]{images/OctaveSettings.png}
\caption{The Octave path settings in the Preferences dialog}
\label{fig:OctaveSetting}
\end{figure}
